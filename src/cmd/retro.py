...
class Retro:
    ...
    def execute(self):
        url = 'https://api.github.com/repos/' + os.environ['gh_organization'] + '/' + self.repo.ghrepo + '/contents/agl/retro'
        response = requests.get(url, auth=(os.environ['user'], os.environ['pass']))
        data = response.json()

        ret = None
        if 'message' in data and 'Not Found' == data['message']:            
            ret = 'Não foi encontrado um time ' + self.repo.ghrepo + ' ou ele não tem retrospectivas no GH :-('
            ...
            if ret is not None:
                ...
                ret = 'O time ' + squadRepo + ' não tem Retrospectivas :-('
